...
/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "cOROSZLAN_22_STUDENTH22Srv.ProductFAQ")
...
try {
    ...
    productFAQ = await SELECT.one('COROSZLAN_22_STUDENTH22.ProductFAQ').where({ ID: productFAQID }).forUpdate();
    if (!productFAQ) {
...
try {
    ...
    await UPDATE('COROSZLAN_22_STUDENTH22.ProductFAQ').set({ embedding: embedding }).where({ ID });
